#!/usr/bin/env python3
"""
Generate synthetic orders from 2025-04-01 to 2025-04-13 with realistic patterns
...
def calculate_base_demand(date):
    """Calculate base demand - starting at 80 orders/day in April"""
    return 80
...
def get_order_status_based_on_age(days_ago):
    ...
    if days_ago == 0:  # Today (April 13)
        return random.choices(['pending', 'confirmed', 'preparing'], weights=[0.4, 0.4, 0.2])[0]
    elif days_ago == 1:  # Yesterday (April 12)
        return random.choices(['confirmed', 'preparing', 'ready', 'delivered'], weights=[0.2, 0.3, 0.2, 0.3])[0]
    elif days_ago == 2:  # April 11
        return random.choices(['preparing', 'ready', 'delivered'], weights=[0.1, 0.3, 0.6])[0]
    elif days_ago <= 5:  # April 8-10
        return random.choices(['ready', 'delivered'], weights=[0.2, 0.8])[0]
    else:  # Older orders (April 1-7)
        return random.choices(['delivered'], weights=[1.0])[0]

def generate_orders_for_period(users, menu_items):
    """Generate orders from April 1-13, 2025"""
    ...
    print("- Realistic order status progression")
    ...
    while current_date <= end_date:
        ...
        if daily_orders > 0:
            print(f"  {current_date.date()} ({current_date.strftime('%A')}): {daily_orders} orders (base: {base_demand}, weekly: {weekly_mult:.1f}x)")
...
def main():
    print("=== April 1-13 Order Generator ===")
    ...
    # Generate orders for the specified period
    orders = generate_orders_for_period(users, menu_items)
